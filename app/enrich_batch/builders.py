...
def build_embedding_requests_docs(
    since_date: Optional[str] = None,
    limit: Optional[int] = None,
    output_path: str = "data/batch/embeddings_docs.requests.jsonl",
    authorities: Optional[List[str]] = None,
    targets_csv_path: str = "data/output/validation/latest/embedding_targets_90d.csv"
) -> Dict:
    """
    Build JSONL request file for embeddings (documents table centric) and write target CSV with chunk info.

    Selects documents joined to events in the last 90 days with missing or legacy embeddings.
    """
    embed_model = os.getenv("EMBED_MODEL", "text-embedding-3-small")
    max_requests = int(os.getenv("BATCH_MAX_REQUESTS", "20000"))
    max_file_mb = int(os.getenv("BATCH_MAX_FILE_MB", "100"))
    ...
    query = """
        SELECT d.document_id, d.clean_text, e.event_id, e.authority
        FROM documents d
        JOIN events e ON e.event_id = d.event_id
        WHERE d.clean_text IS NOT NULL AND LENGTH(d.clean_text) > 100
    ...
    # Target CSV header
    import csv as _csv
    with open(targets_csv_path, "w", newline="", encoding="utf-8") as tf, open(output_path, "w", encoding="utf-8") as jf:
        tw = _csv.writer(tf)
        tw.writerow(["document_id","event_id","authority","chunk_index","estimated_tokens","chunk_text"])
        for doc_id, clean_text, event_id, authority in rows:
            chunks = chunk_text(clean_text, max_tokens=1500, overlap_pct=0.10)
            for idx, chunk in enumerate(chunks):
                tok = estimate_tokens(chunk, "text-embedding-3-small")
                total_tokens += tok
                tw.writerow([doc_id, event_id, authority, idx, tok, chunk])
                req = {
                    "custom_id": f"emb:{doc_id}:{idx}",
                    "method": "POST",
                    "url": "/v1/embeddings",
    ...
    # Cost estimate (text-embedding-3-small: $0.00002 per 1K tokens; 50% batch discount)
    cost_per_1k_tokens = 0.00002
    projected_cost = (total_tokens / 1000.0) * cost_per_1k_tokens * 0.50
